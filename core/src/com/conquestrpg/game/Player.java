...
public class Player {
    ...
    private static final String _defaultSpritePath = "Main.png";
    ...
    public void setBoundingBoxSize(float percentageWidthReduced, float percentageHeightReduced){
        ...
        if( width == 0 || height == 0){
            ...
            //Need to account for the unitscale, since the map coordinate will be in pixels
            float minX;
            float minY;
    ...
    private void loadAllAnimations(){
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if( region == null ){
                    Gdx.app.debug(TAG, “Got null animation frame “ + i +
               “,” + j);
                } switch(i) {
                    case 0:
